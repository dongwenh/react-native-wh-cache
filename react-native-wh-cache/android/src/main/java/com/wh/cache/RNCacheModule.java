...
public class RNCacheModule extends ReactContextBaseJavaModule {
  ...
  public RNCacheModule(ReactApplicationContext reactContext) {
    super(reactContext);
  ...
  @ReactMethod
    public void getHttpDiskCacheSize(Promise promise){
        try {
    ...
    @ReactMethod
    public void getHttpMemoryCacheSize(Promise promise){
        promise.resolve(0);
    }
    @ReactMethod
    public void clearHttpAllCache(Promise promise){
        try {
            Cache cache = OkHttpClientProvider.getOkHttpClient().cache();
            if (cache != null){
                cache.delete();
